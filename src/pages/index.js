...
export default function Home() {
    ...
    const handlePlayerSearchInput = async (e) => {
        ...
        if (account.length < 2) {
            alert(
                `Please provide a discriminator (# symbol followed by 3-6 digits)`
            );
            return;
        }
        ...
        if (account[0].length < 3 || account[0].length > 12) {
            alert(
                `Account "${account[0]}" must be between 3 and 12 characters.`
            );
            return;
        }
        ...
        if (
            showAccounts.filter((account) => account.name == playerSearch)
                .length > 0
        ) {
    ...
    async function updateAccount(id) {
        ...
        const updatedAccount = await getPlayer(buffer.name);

        let updatedAccountList = showAccounts
        updatedAccountList[id] = updatedAccount
        setShowAccounts(updatedAccountList); // Refresh the account list
        saveToLocal(updatedAccountList);
    }
                                ...
                                >
                                    ...
                                    >
                                        <img
                                            ...
                                            width="40"
                                        ></img>
                                        <h5 className="player-name card-header">
                                            {account.name}
                                        </h5>
                                    </div>
                                    <div
